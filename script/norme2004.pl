#!/usr/bin/perl -w
## chk_style.pl for chk_style in /u/a1/vasseu_c
##
## Made by clement vasseur
## Login   <vasseu_c@epita.fr>
##
## Modified by <demang_n>
##
## Started on  Tue Oct  8 00:38:10 2002 clement vasseur
## Last update Thu Oct  2 18:11:30 2003 nicolas demange
##
...
sub usage {
    print "chk_style.pl - check EPITA coding style\n";
    print "written by Clement Vasseur (nitro\@epita.fr)\n\n";
    print "Usage: chk_style.pl [-fhv] file.c [...]\n";
...
sub is_type {
    ...
    return err("invalid type '$t'") unless
        $t =~ m/^(const )?(static )?(const )?(unsigned )?(short )?(long )?(void|int|char|long|short|float|double|[a-zA-Z0-9_]+|struct +[a-zA-Z0-9_]+)$/;
    return 1;
}
